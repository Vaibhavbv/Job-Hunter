...
import type { Evaluation, Grade, Archetype, Recommendation } from '../types/database'
...
export function useEvaluations() {
  ...
  const [progress, setProgress] = useState<EvaluateProgress>({ current: 0, total: 0, message: '' })
  ...
  const evaluate = useCallback(
    async (jobIds: number[] = [], sessionId: string | null = null) => {
      setEvaluating(true)
      let totalNew = 0
      ...
      try {
        if (jobIds.length > 0) {
          ...
          while (round < MAX_ROUNDS) {
            ...
            setProgress((prev) => ({
              ...prev,
              message: `Batch ${round}: evaluating up to 20 jobs...`,
            }))
            ...
            setProgress({
              ...
              message: batchNew > 0 ? `Evaluated ${totalNew} jobs so far...` : 'All jobs evaluated!',
            })
  ...
  const stats = useMemo(() => {
    if (evaluations.length === 0) {
      return {
        ...
        topJobs: [] as Evaluation[],
        gateFailCount: 0,
      }
    }

    const scores = evaluations.map((e) => e.overall_score).filter(Boolean) as number[]
    const avgScore =
    ...
    // Top 5 jobs
    const topJobs = evaluations.filter((e) => !e.gate_fail).slice(0, 5)
  ...
  const getByGrade = useCallback(
    (grade: Grade) => evaluations.filter((e) => e.grade === grade),
    [evaluations],
  )

  const getByArchetype = useCallback(
    (archetype: Archetype) => evaluations.filter((e) => e.archetype === archetype),
    [evaluations],
  )

  const getByRecommendation = useCallback(
    (recommendation: Recommendation) =>
      evaluations.filter((e) => e.recommendation === recommendation),
    [evaluations],
  )

  const getTopN = useCallback(
    (n = 10) => evaluations.filter((e) => !e.gate_fail).slice(0, n),
    [evaluations],
  )
